...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    const createMicButton = () => {
      // Inject into the controls container (same parent as send button) to avoid overlay blocking clicks
      const controls = chatContainer.querySelector('.chat-inputs-controls')
        || chatContainer.querySelector('.chat-input')
        || chatContainer.querySelector('.chat-inputs');
      if (!controls || controls.querySelector('.atlas-mic-button')) return;
      ...
      // Insert before the send button so mic appears to its left
      const sendButton = controls.querySelector('button:not(.atlas-mic-button)');
      if (sendButton) {
        controls.insertBefore(micButton, sendButton);
      } else {
