...
export const CareerSignatureCard: React.FC<CareerSignatureCardProps> = ({ reportId, className }) => {
  ...
  return (
    <div className={className}>
      ...
      >
        {/* Header */}
        <div className="px-5 sm:px-7 pt-6 pb-4 flex items-center justify-between">
          ...
          <span className="text-[11px] uppercase tracking-wider font-semibold text-atlas-navy/50">
            ATLAS
          </span>
        </div>
        ...
        <div className="px-5 sm:px-7 pb-6">
          <div className="text-[10px] uppercase tracking-[0.2em] font-semibold text-gray-500 mb-2">
            Strongest Match {firstName ? `· ${firstName}` : ''}
          </div>
          <h2 className="font-heading text-[1.65rem] sm:text-[2rem] leading-tight font-bold text-atlas-navy mb-3">
            {hero.title}
          </h2>
          <div className="flex flex-wrap items-center gap-2">
            <MatchPill score={hero.score} size="lg" />
            {hero.aiImpact && <ImpactPill level={hero.aiImpact} size="lg" />}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-atlas-navy/10">
          {[second, third].map((c, i) =>
            c ? (
              <div key={c.rank} className="px-5 sm:px-7 py-5">
                <div className="text-[10px] uppercase tracking-[0.2em] font-semibold text-gray-500 mb-1.5">
                  {c.rank === 2 ? 'Runner-up #1' : 'Runner-up #2'}
                </div>
                <div className="font-heading text-[1.05rem] sm:text-[1.15rem] leading-snug font-semibold text-atlas-navy mb-2.5 line-clamp-2">
                  {c.title}
                </div>
              ...
              <div key={`empty-${i}`} className="px-5 sm:px-7 py-5 opacity-50">
                <div className="text-[10px] uppercase tracking-[0.2em] font-semibold text-gray-400 mb-1.5">
                  {i === 0 ? 'Runner-up #1' : 'Runner-up #2'}
                </div>
        ...
        {/* Footer */}
        <div className="border-t border-atlas-navy/10 px-5 sm:px-7 py-3.5 flex items-center justify-between bg-white/40">
          <span className="text-[11px] text-atlas-navy/60">
            {totalScored > 0
              ? `${totalScored} role${totalScored === 1 ? '' : 's'} analyzed against your profile`
              : 'Generated from your Atlas Assessment'}
          </span>
          <span className="text-[11px] font-semibold text-atlas-teal">
            atlas-assessments.com
          </span>
        </div>
      </div>
    </div>
