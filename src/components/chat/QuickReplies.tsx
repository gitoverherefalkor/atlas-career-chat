...
export const QuickReplies: React.FC<QuickRepliesProps> = ({ onSend, onFocusInput, visible }) => {
  ...
  return (
    <div className="flex flex-wrap gap-2 mt-3 mb-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
      {REPLIES.map((reply) => (
        <button
          key={reply.label}
          onClick={() => handleClick(reply)}
          className="inline-flex items-center gap-1.5 px-3.5 py-2 text-sm font-medium rounded-full
